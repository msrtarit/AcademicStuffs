...
lowest_cost_per_day = float('inf')  # Initialize with positive infinity
best_machine_capacity = 0  # Initialize to keep track of the best machine capacity
...
while machine_capacity <= 200:
    ...
    # Calculate simulation results for the current machine capacity
    total_cost = (cumulative_idle_capacity * cost_per_idle_hour) + (cumulative_waiting_time * cost_per_waiting_hour)
    cost_per_day = total_cost / simulation_days
    
    # Print simulation results for the current machine capacity
    print("\n Capacity =", machine_capacity, "Total cost =", total_cost, "Cost per day =", cost_per_day)
    
    # Update lowest cost per day and best machine capacity if necessary
    if cost_per_day < lowest_cost_per_day:
        lowest_cost_per_day = cost_per_day
        best_machine_capacity = machine_capacity
    
    machine_capacity += 10  # Increment machine capacity for the next iteration
    print()

# Print the lowest cost per day and the corresponding best machine capacity after the simulation
print("\n Lowest cost per day:", lowest_cost_per_day)
print(" Best machine capacity:", best_machine_capacity)
